...
NoteA Demo of vim.pack (Neovim built-in plugin manager)

![](https://www.youtube.com/watch?v=J1r0vrqOMJo)
...
![](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack/demo-vim-pack-add-confirm.png)

Adding a plugin via `vim.pack.add()` makes it “active” or “loaded” in current session.

---

All plugins that were not previously installed by `vim.pack` are installed in parallel. They are put into a dedicated `core` [plugin package](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#plugin-packages) as “opt” plugins (see [`:h vim.pack-directory`](https://neovim.io/doc/user/helptag.html?tag=vim.pack-directory)). There is no “start” plugin support because:
...
- If not already, set up a directory for “local” plugins that fits into [plugin package](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#plugin-packages) structure. The easiest approach is a dedicated package like `~/.local/share/nvim/site/pack/mine/opt` or `~/.config/nvim/pack/mine/opt` (while adding `pack/` directory to `.gitignore`). It can also be anywhere on the disk but with a symlink to an “opt” part of a known plugin package.
...
- Not [update](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#update) the plugin with `vim.pack`. It will change the state to where its `version` points, so local changes will not have effect.
...
- It can be called multiple times with different plugin lists (with [minor caveats](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#many-vim-pack-add)) for a more “modular” config.
- [Lazy loading](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lazy-loading) in principle is just “call this exact function, just some time later”.
- It can be used interactively like `:lua vim.pack.add({ 'https://github.com/nvim-mini/mini.nvim' })`.

---

By default loading plugin is done by executing `:packadd` (possibly with `!`) command, which essentially makes plugin’s [runtime files](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#runtime-files) known to Neovim and sources some of them (`plugin/` and `ftdetect/` scripts).

With `vim.pack.add()` it is also possible to have custom loading logic. This can be useful to do something more or do nothing at all. The latter is useful as a workaround for some complex [lazy loading](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lazy-loading). For example:
...
Do nothing instead of `:packadd` to load
...
```lua
vim.pack.add({
1  { src = 'https://github.com/nvim-mini/mini.nvim', version = 'stable' },
2  { src = 'https://github.com/neovim/nvim-lspconfig', name = 'lspconfig' },
...
Use updates from `stable` Git reference (branch, tag, or commit)
...
Table and string specifications can be mixed
...
1    version = vim.version.range('*')
  },
  {
    src = 'https://github.com/neovim/nvim-lspconfig',
2    version = vim.version.range('2.x')
  },
})
```
...
Always try installing latest semver tag

2

Install latest semver tag if it is >=2.0.0 and <3.0.0

Use `data` field to store/pass extra information or plugin specific methods:

```lua
local selective_load = function(plug_data)
1  if (plug_data.spec.data or {}).skip_load then return end
...
Do not load if explicitly asked
...
- As a consequence of the previous point, it makes it possible to ensure that even [lazy loaded](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lazy-loading) plugins are installed right away at the expected state and not when their first `vim.pack.add` ed.
- It enables easier “revert the latest update” workflow by reverting the lockfile and [updating to the lockfile’s state](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#update).
- It contains information about plugin’s `version`, i.e. target state when updating. This is useful when [updating](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#update) if there are not-yet-active plugins (since there is no `vim.pack.add()` call yet to say which `version` it follows).
...
If something is done to plugins not via `vim.pack` methods, lockfile can become out of sync with what is present on disk. Following [troubleshooting steps](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#troubleshoot) should usually help.
...
1  if name == 'nvim-treesitter' and kind == 'update' then
2    if not ev.data.active then vim.cmd.packadd('nvim-treesitter') end
    vim.cmd('TSUpdate')
  end
end })
```
...
Make sure that ‘nvim-treesitter’ is loaded to use `:TSUpdate`
...
- (Recommended) Create an autocommand before **the very first `vim.pack.add()`**. This will run the hook even when installing the plugin [based on the lockfile](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lockfile).
- If the hook *absolutely needs* to have plugin’s `data` (like if it contains project build instructions), the plugin should be listed in the very first `vim.pack.add()`. If this plugin is intended to be lazy loaded, [register it without loading](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#vim-pack-add-no-load) and use `vim.cmd.packadd` when later loading.

## Config organization

### Single vim.pack.add()

This is the most robust approach around which `vim.pack` was designed. Have a single `vim.pack.add()` that installs+loads plugins and (if needed) create one autocommand for hooks prior to it. Also works well when “bootstrapping” config on new machine (since it automatically resolves possible [installation hooks](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#hooks-install) problems).
...
3vim.cmd.colorscheme('miniwinter')
...
Define hooks via autocommand(s)

2

Add (maybe install and load) all at once
...
- It is not “modular” enough as many people prefer spreading plugin configuration across multiple files. The usual answer is to use [many `vim.pack.add()` calls](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#many-vim-pack-add).
- It loads all plugins immediately during startup which can visibly affect startup time. It also increases ‘runtimepath’ (which can have [slightly negative impact](https://echasnovski.com/blog/2026-01-27-how-many-neovim-plugins-is-too-many.html)). The answer is to use [lazy loading](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lazy-loading), but **please** only to a moderate extent. Extreme lazy loading usually comes with a hidden cognitive overhead both when using and maintaining the config.
...
The current single fragility of this approach occurs when “bootstrapping” from the [lockfile](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lockfile) a plugin that requires an action during install. See [dedicated note](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#hooks-install) about possible solutions.
...
vim.schedule(function()
  vim.pack.add({
1    'https://github.com/nvim-mini/mini.cmdline',
...
```lua
1vim.api.nvim_create_autocmd('CmdlineEnter', { once = true, callback = function()
  vim.pack.add({ 'https://github.com/nvim-mini/mini.cmdline' })
2  require('mini.cmdline').setup()
end })

3vim.api.nvim_create_autocmd('InsertEnter', { once = true, callback = function()
...
Load once when entering Command-line mode

2

Explicitly set up ‘mini.nvim’ module

3

Load once when entering Insert mode
...
```lua
1vim.pack.add({ 'https://github.com/nvim-mini/mini.misc' })
...
2later(function()
  vim.pack.add({ 'https://github.com/nvim-mini/mini.cmdline' })
3  require('mini.cmdline').setup()
end)

4on_event('InsertEnter', function()
  vim.pack.add({ 'https://github.com/nvim-mini/mini.completion' })
  require('mini.completion').setup()
end)
```
...
Load some time later after startup
...
- This also has the same “install hooks” fragility as the [many `vim.pack.add()`](https://echasnovski.com/blog/many-vim-pack-add) approach with the same solutions.
...
![](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack/demo-confirmation-buffer.png)
...
- There is a so-called “in-process” (i.e. created in Neovim session with Neovim’s Lua code) LSP server enabled specifically in confirmation buffer. It provides several features in an LSP compatible way. This means that they will work both with [default LSP mappings](https://neovim.io/doc/user/helptag.html?tag=lsp-defaults) and with any custom mappings.
	...
	- [‘textDocument/documentSymbol’](https://neovim.io/doc/user/helptag.html?tag=vim.lsp.buf.document_symbol\(\)) to show structure of the buffer. By default opens a location list, but will work with, for example, any picker providing LSP document symbols picker.
		...
		- [‘textDocument/codeAction’](https://neovim.io/doc/user/helptag.html?tag=vim.lsp.buf.code_action\(\)) - show code actions relevant for a “plugin at cursor”. Like “delete” (if plugin is not active; useful if you only removed plugin from config but did not [delete it](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#delete)), “update” or “skip updating” (if there are pending updates).
...
- `:lua vim.pack.update(nil, { target = 'lockfile' })` - make sure that plugin state on disk is the same as recorded in the [lockfile](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lockfile). Useful for reverting updates: ensure the lockfile is as it was before the update and execute the command.
...
- Use [`vim.pack.del()`](https://neovim.io/doc/user/helptag.html?tag=vim.pack.del\(\)) to delete plugin(s) from disk. This will also remove the plugin from the [lockfile](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lockfile). Here is an example: `:lua vim.pack.del({ 'nvim-lspconfig', 'nvim-treesitter' })`.
...
- Detect missing or problematic [lockfile](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lockfile) entry.
- Whether lockfile is not aligned with what is on the disk.
- Notify if there are not active (installed but not loaded) plugins. Which might be a sign that it was removed from the config but not [fully deleted](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#delete).
...
- Start with a simple [one `vim.pack.add()`](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#single-vim-pack-add) style of config and see if it fits your needs. It might be surprising and refreshing to trade the startup speed for config simplicity.
	If it doesn’t feel right, you can start [modularizing](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#many-vim-pack-add) the config. And only if absolutely needed, try [lazy loading](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lazy-loading) some plugins.
...
- Rewrite hooks to be [in autocommand form](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#hooks).
...
- **Do not forget** to remove plugins (located in `site/pack/deps` subdirectory of [“data” standard path](https://neovim.io/doc/user/helptag.html?tag=standard-path)) and possible `mini-deps-snap` snapshot file. Otherwise there will be conflicts (since ‘mini.deps’ also uses [plugin package](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#plugin-packages) approach).
...
By design, not all ‘lazy.nvim’ capabilities are one-to-one reproducible in `vim.pack`. My best suggestion would be to follow [both approaches suggested earlier](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#migrate): use temporary config and start with [simple approach](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#single-vim-pack-add) first to see that maybe there is nothing much to miss from ‘lazy.nvim’.
...
- There is no built-in lazy loading with `vim.pack`, so specification fields like `cmd`, `event`, `ft`, `keys` can not be translated directly. For emulating loading on event see [“Lazy loading”](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lazy-loading) section.
...
- The most speedup usually comes from [lazy loading](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#lazy-loading), at the cost of extra a more complex config. Choosing the right balance between the two is up to you.
...
The most recommended way of setting up ‘lazy.nvim’ plugins seems to be a so called [“structuring plugins”](https://lazy.folke.io/usage/structuring) approach. Beware that those `lua/plugins/` plugin files are discovered and sourced by ‘lazy.nvim’ automatically. Just translating each file to use `vim.pack.add()` will not be enough, make sure to source those files during startup. One approach that I can recommend is to move them from `lua/plugins/` to `plugin/` directory (notice the singular case). The `plugin/` [runtime files](https://echasnovski.com/blog/2026-03-13-a-guide-to-vim-pack.html?utm_source=chatgpt.com#runtime-files) are sourced automatically during startup by Neovim itself.
...
[Reply on Bluesky](https://bsky.app/profile/did:plc:sovgyj2emvmz4dp4naq7wy3l/post/3mgxaiqbrts2y) to join the conversation.